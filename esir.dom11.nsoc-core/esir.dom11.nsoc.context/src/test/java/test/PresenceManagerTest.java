...
import esir.dom11.nsoc.context.presence.PresenceCalendarEvent;
...
public class PresenceManagerTest extends TestCase {
    ...
    public void testGeneral() {
        ...
        presMan.getCepRT().sendEvent(new PresenceCalendarEvent("1", true));
        presMan.getCepRT().sendEvent(new PresenceEvent("1", true));
        tempo(2000);
        presMan.getCepRT().sendEvent(new PresenceEvent("1", false));
        tempo(2000);
        presMan.getCepRT().sendEvent(new PresenceCalendarEvent("1", false));
        tempo(2000);
        presMan.getCepRT().sendEvent(new PresenceEvent("1", false));
        tempo(2000);
        presMan.getCepRT().sendEvent(new PresenceEvent("1", true));
